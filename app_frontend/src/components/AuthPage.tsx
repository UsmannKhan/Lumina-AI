...
import Input from './Input';
import { UserIcon, SparklesIcon } from './Icons';
import { BookOpen, Brain, MessageCircle, FileText } from 'lucide-react';
import clsx from 'clsx';
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 xl:w-3/5 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center p-12 xl:p-20">
          ...
          <div className="mt-12 grid grid-cols-2 gap-4">
            {[
              { icon: FileText, label: 'Smart Notes', desc: 'Auto-generated summaries' },
              { icon: BookOpen, label: 'Flashcards', desc: 'Test your recall' },
              { icon: Brain, label: 'Quizzes', desc: 'Prove your knowledge' },
              { icon: MessageCircle, label: 'Chat', desc: 'Ask anything' },
            ].map((feature) => (
              <div 
                key={feature.label} 
                className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]"
              >
                <feature.icon size={20} className="text-ember-400 mb-2" />
                <p className="text-white font-medium text-sm">{feature.label}</p>
                <p className="text-void-500 text-xs mt-0.5">{feature.desc}</p>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <p className="mt-8 text-center text-sm text-void-500">
